...
void CheckUObjectIsNull(const UObject* InObject, const FString& InTag)
{
	if (nullptr == InObject)
	{
		UE_LOG(LogTemp, Log, TEXT("[%s] 널 포인트 언리얼 오브젝트"), *InTag);
	}
	else
	{
...
void UMyGameInstance::Shutdown()
{
	...
	CheckUObjectIsNull(NonPropStudents[0], TEXT("NonPropStudent"));
	CheckUObjectIsValid(NonPropStudents[0], TEXT("NonPropStudent"));

	CheckUObjectIsNull(PropStudents[0], TEXT("PropStudent"));
	CheckUObjectIsValid(PropStudents[0], TEXT("PropStudent"));
}