...
void UMyGameInstance::Init()
{
	...
	{
		// Add나 Emplace와 성능은 거의 비슷하지만 가독성은 Add 성능은 Emplace를 사용하는 것이 좋습니다.
		Int32Array.Add(ix);
	}
	...
	Algo::Transform(StudentsData, StudnetsMapByUniqueName, [](const FStudentData& Val)
		{
			return TPair<FString, int32>(Val.Name, Val.Order);
		});

	UE_LOG(LogTemp, Log, TEXT("이름에 따른 학생 맵의 레코드 수 : %d"), StudnetsMapByUniqueName.Num());

	TMultiMap<FString, int32> StudentMapByName;
	Algo::Transform(StudentsData, StudentMapByName, [](const FStudentData& Val)
		{
			return TPair<FString, int32>(Val.Name, Val.Order);
		});
